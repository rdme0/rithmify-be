...
**Spring Boot (MVC)**와 **Kotlin**을 기반으로 하며, **가상 스레드(Virtual Threads)**와 **코루틴(Coroutines)**을 적극 활용하여 고성능 처리를 지향합니다.

## 🤖 AI 개발자 가이드 (핸드오버 노트)
이 문서는 추후 프론트엔드 개발이나 유지보수를 담당할 AI 및 개발자를 위해 작성되었습니다.
**핵심 목표**: React/Next.js 프론트엔드와 연동될 견고하고 빠른 REST API 서버 구축.
